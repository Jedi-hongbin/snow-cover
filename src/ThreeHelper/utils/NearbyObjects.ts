...
export class NearbyObjects {
    ...
    filterObject(...args: Object3D[]) {
        args.forEach((obj) => {
            obj.traverse((item: { userData: { filter: boolean; }; }) => {
    ...
    nearbyMesh(position: Vector3, distance: number) {
        ...
        this._object.traverse((obj: { userData: { _box3: any; filter: any; }; position: { x: number; z: number; }; }) => {
    ...
    isCollider(target: Object3D, targetBox3: Box3, distance: number) {
        ...
        this._object.traverse((obj: { userData: { _box3: { intersectsBox: (arg0: Box3) => any; }; }; position: Vector3Like; }) => {
            if (hit) return;
